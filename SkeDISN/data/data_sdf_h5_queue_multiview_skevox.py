...
class Pt_sdf_img(threading.Thread):
    ...
    def getitem(self, index):
        cat_id, obj, num_list = self.listinfo[index]
        ...
        return ori_pt, ori_sdf_val, sample_pt, sample_sdf_val, norm_params,\
               sdf_params, img_dir, img_file_lst, cat_id, obj, num_list, vox64_dir, vox128_dir, vox256_dir
    ...
    def get_img(self, img_dir, num_list):
        multi_img_arr, multi_cam_mat, multi_cam_pos, multi_trans_mat,\
            multi_obj_rot_mat, mulit_regress_mat = [], [], [], [], [], []
        if num_list is None:
            num_list = random.sample(range(24), self.view_num)
        for num in num_list:
            ...
            with h5py.File(img_h5, 'r') as h5_f:
                if self.FLAGS.img_feat_onestream or self.FLAGS.img_feat_twostream or self.FLAGS.img_feat_threestream:
                    ...
                    cam_mat, cam_pos = np.zeros((3,3), dtype='f4'), np.zeros((3,), dtype='f4')
                if self.FLAGS.alpha:
            ...
            multi_img_arr.append(img_arr)
            multi_cam_mat.append(cam_mat)
            multi_cam_pos.append(cam_pos)
            multi_trans_mat.append(trans_mat)
            multi_obj_rot_mat.append(obj_rot_mat)
            mulit_regress_mat.append(regress_mat)
        multi_img_arr = np.stack(multi_img_arr, axis=0)
        multi_trans_mat = np.stack(multi_trans_mat, axis=0)
        multi_obj_rot_mat = np.stack(multi_obj_rot_mat, axis=0)
        mulit_regress_mat = np.stack(mulit_regress_mat, axis=0)
        return multi_img_arr, multi_cam_mat, multi_cam_pos, multi_trans_mat,\
            multi_obj_rot_mat, mulit_regress_mat
    ...
    def get_batch(self, index):
        ...
        if self.FLAGS.alpha:
            batch_img = np.zeros((self.batch_size, self.view_num, self.FLAGS.img_h, self.FLAGS.img_w, 4), dtype=np.float32)
        else:
            batch_img = np.zeros((self.batch_size, self.view_num, self.FLAGS.img_h, self.FLAGS.img_w, 3), dtype=np.float32)
        batch_regress_mat = np.zeros((self.batch_size, self.view_num, 4, 3), dtype=np.float32)
        batch_trans_mat = np.zeros((self.batch_size, self.view_num, 4, 3), dtype=np.float32)
        ...
        for i in range(index, index + self.batch_size):
            ...
            ori_pt, ori_sdf_val, sample_pt, sample_sdf_val, norm_params, sdf_params, img_dir, \
                img_file_lst, cat_id, obj, num_list,vox64_dir, vox128_dir, vox256_dir = single_obj
            img, cam_mat, cam_pos, trans_mat, obj_rot_mat, regress_mat = self.get_img(img_dir, num_list)
            if ori_pt is not None:
                vox64 = self.get_vox64(vox64_dir, num_list)
                vox128 = self.get_vox128(vox128_dir, num_list)
                vox256 = self.get_vox256(vox256_dir, num_list)
                #[262144, 4, 4, 4]
