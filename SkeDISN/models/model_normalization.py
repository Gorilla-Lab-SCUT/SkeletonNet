...
def get_loss(end_points, sdf_weight=10., regularization=True, mask_weight = 4.,
    ...
    if FLAGS.binary:
        if not FLAGS.in1out0:
            label_sdf = tf.reshape(tf.cast(tf.greater(gt_sdf, tf.constant(0.0)), dtype=tf.int32),
                               (batch_size, num_sample_points))
        else:
            label_sdf = tf.reshape(tf.cast(tf.less(gt_sdf, tf.constant(0.0)), dtype=tf.int32), 
                                    (batch_size, num_sample_points))
        ...
        end_points['losses']['sdf_loss_realvalue'] = tf.reduce_mean(sdf_loss * tf.squeeze(weight_mask, axis=2))
