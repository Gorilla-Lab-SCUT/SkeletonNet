...
def placeholder_inputs(batch_size, num_points, img_size, num_sample_pc = 256, scope='', FLAGS=None):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        if FLAGS.grayscale:
            imgs_pl = tf.placeholder(tf.float32, shape=(batch_size, img_size[0], img_size[1], 1))
        else:
    ...
    if FLAGS.binary:
        if FLAGS.img_feat_twostream:
            ...
            with tf.variable_scope("sdfprediction") as scope:
                ref_sample_pc_rot_pointfeat, pred_sdf_value_global = sdfnet.get_sdf_basic2_share_binary(ref_sample_pc_rot, ref_feats_embedding_cnn,
            ...
            if FLAGS.ske_local_patch_share:
                batch_size = ref_sample_pc.get_shape()[0].value
                num_sample_pc = ref_sample_pc.get_shape()[1].value
            ...
            if not FLAGS.multi_view:
                with tf.variable_scope("sdfprediction") as scope:
                    ref_sample_pc_rot_pointfeat, pred_sdf = sdfnet.get_sdf_basic2_share_binary(ref_sample_pc_rot, ref_feats_embedding_cnn,
                            is_training, batch_size, num_point, bn, bn_decay,wd=wd)
            if FLAGS.ske_local_patch_share:
                batch_size = ref_sample_pc.get_shape()[0].value
                num_sample_pc = ref_sample_pc.get_shape()[1].value
                ...
                with tf.variable_scope("sdfprediction_voxfeat") as scope:
                    pred_sdf_value_local_vox = sdfnet.get_sdf_basic2_imgfeat_twostream_share_binary(ref_sample_pc_rot_pointfeat, point_vox_feat, 
                        is_training, batch_size, num_point, bn, bn_decay, wd=wd)
                ##############
                pred_sdf = pred_sdf + pred_sdf_value_local_vox
