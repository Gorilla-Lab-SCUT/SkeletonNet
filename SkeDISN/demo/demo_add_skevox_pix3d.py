...
import model_add_skevox_pix3d as model
...
def read_img_get_transmat():
    ...
    for i in range(len(data_list)):
        ...
        if cat == FLAGS.cat and img_path[:-4] == FLAGS.demo_image_path[:-4]:
            ...
            if False:
                mesh_list = trimesh.load_mesh('/data/tang.jiapeng/pix3d/'+model_path, process=False)
                ...
                points_all = points_all - center
                m = np.max(np.sqrt(np.sum(points_all ** 2, axis=1)))
            else:
                mesh = openmesh.read_trimesh('/data/tang.jiapeng/pix3d/'+model_path)
                points = mesh.points()
                center = np.zeros((3), dtype='float32')
                m = np.max(np.sqrt(np.sum(points ** 2, axis=1)))
                min_box = np.min(points, axis=0)
                max_box = np.max(points, axis=0)
                box_len = np.max(max_box - min_box) / m * 1.2
            ...
            norm_mat = np.matmul(T_inv, M_inv)
            ...
            continue
    trans_mat = np.linalg.multi_dot([K, RT, norm_mat, rot_mat])
    ...
    batch_data['norm_params'] = np.array([[center[0], center[1], center[2], m]])
    return batch_data
