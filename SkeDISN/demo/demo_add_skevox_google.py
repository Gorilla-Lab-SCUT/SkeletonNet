...
parser.add_argument('--demo_cam_path', type=str, default="", help='demo_cam_path')
parser.add_argument('--demo_skevox_path', type=str, default="", help='demo_skevox_path')
#
parser.add_argument('--in1out0', action='store_true')
parser.add_argument('--weight_skevox', type=float, default=1.0)
...
if FLAGS.cam_est:
    if FLAGS.weight_skevox > 1.0:
        RESULT_OBJ_PATH = os.path.join("./demo/google_images_add_skevox_w%d"%int(FLAGS.weight_skevox))
    else:
        RESULT_OBJ_PATH = os.path.join("./demo/google_images_add_skevox/")
    ...
    if FLAGS.ske_local_patch or FLAGS.ske_local_patch_share:
        ...
        RESULT_OBJ_PATH = os.path.join("./demo/google_images_no_skevox/")
        print("RESULT_OBJ_PATH: ",RESULT_OBJ_PATH)
...
def read_img_get_transmat():
    ...
    vox_dir = FLAGS.demo_skevox_path
    ...
    if FLAGS.cam_est:
        ...
        batch_data['trans_mat'] = np.transpose(np.zeros((3, 4), dtype='float32'))[None, :, :]
    ...
    cam_path = FLAGS.demo_cam_path
    cam_info = loadmat(cam_path)
    scale, translate = cam_info["scale"], cam_info["translate"]
    batch_data['norm_params'] = np.array([[translate[0, 3], translate[1, 3], translate[2, 3], 
                                          1.0/scale[0, 0], 1.0/scale[1, 1], 1.0/scale[2, 2]]], dtype='float32')
    return batch_data
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        if not FLAGS.threedcnn:
            for b in range(BATCH_SIZE):
                ...
                center, m, = norm_params[0:3], norm_params[3:6]
                print(center, m, center.shape, m.shape)
                ...
                for j in range(SPLIT_SIZE):
                    ...
                    split_pointsf = (split_pointsf - center[None,:]) * m[None, :]  ### align with shapenet v1
