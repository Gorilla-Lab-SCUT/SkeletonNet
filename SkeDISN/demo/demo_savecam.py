...
parser.add_argument('--demo_mask_path', type=str, default="", help='demo_mask_path')
parser.add_argument('--demo_basemesh_path', type=str, default="", help='demo_image_path')
parser.add_argument('--maskcolorwhite', action='store_true')
...
if FLAGS.cam_est:
    RESULT_OBJ_PATH = os.path.join("./demo/google_images")
...
def read_img_get_transmat():
    ...
    print(img_arr.shape, img_arr[0, 0])
    img_arr = img_arr[:, :, :3]
...
def test_one_epoch(sess, ops, batch_data):
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        if FLAGS.binary:
            ...
            result = (prob - FLAGS.threshold) #/10.0
        ...
        for b in range(BATCH_SIZE):
            ...
            executor.submit(create_obj, result[b], batch_data['sdf_params'][b], RESULT_OBJ_PATH,
                FLAGS.iso)
        ###save cam mat and scale, translation
        for b in range(BATCH_SIZE):
            print("save cammat scale an translation")
            obj_nm = (FLAGS.demo_image_path).split('/')[-1][:4]
            cube_obj_file = os.path.join(RESULT_OBJ_PATH, obj_nm+".obj")
            baemesh_obj_file =  FLAGS.demo_basemesh_path
            cubemesh = trimesh.load(cube_obj_file)
            basemesh = trimesh.load(baemesh_obj_file)
            cubemesh_minbox = np.min(cubemesh.vertices, axis=0)
            cubemesh_maxbox = np.max(cubemesh.vertices, axis=0)
            cubemesh_sizelen = cubemesh_maxbox - cubemesh_minbox
            cubemesh_centre = (cubemesh_maxbox + cubemesh_minbox) * 0.5
            basemesh_minbox = np.min(basemesh.vertices, axis=0)
            basemesh_maxbox = np.max(basemesh.vertices, axis=0)
            basemesh_sizelen = basemesh_maxbox - basemesh_minbox
            basemesh_centre = (basemesh_maxbox + basemesh_minbox) * 0.5
            scale_ratio = cubemesh_sizelen / basemesh_sizelen
            cam_file = os.path.join(RESULT_OBJ_PATH, obj_nm+".mat")
            save_dict = {"trans_mat": batch_data['trans_mat'][b],
                         "scale": np.asarray(
                                [[scale_ratio[0], 0., 0., 0.],
                                 [0., scale_ratio[1], 0., 0.],
                                 [0., 0., scale_ratio[2], 0.],
                         ...
                         "translate": np.asarray(
                                [[1.0 , 0., 0., cubemesh_centre[0]],
                                 [0., 1.0 , 0., cubemesh_centre[1]],
                                 [0., 0., 1.0 , cubemesh_centre[2]],
                                 [0., 0., 0., 1.]], dtype='float32'
                            )}
            savemat(cam_file, save_dict)
