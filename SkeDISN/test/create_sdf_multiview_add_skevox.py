...
import model_add_skevox_multiview as model
from concurrent.futures import ThreadPoolExecutor
import data_sdf_h5_queue_multiview_skevox as data
...
parser.add_argument('--img_path', type=str, default="demo_image_path")
...
if FLAGS.img_feat_twostream:
    SPLIT_SIZE = int(np.ceil(TOTAL_POINTS / 114669.0))
elif FLAGS.threedcnn :
    SPLIT_SIZE = 1
else:
    SPLIT_SIZE = int(np.ceil(TOTAL_POINTS / 100000))#274625.0))
NUM_SAMPLE_POINTS = int(np.ceil(float(TOTAL_POINTS) / SPLIT_SIZE))
GPU_INDEX = FLAGS.gpu
...
if FLAGS.test_allset:
    ...
    RESULT_OBJ_PATH = os.path.join(LOG_DIR, 'demo_objs', str(RESOLUTION) + "_" + str(FLAGS.iso))
    if not os.path.exists(RESULT_OBJ_PATH): os.makedirs(RESULT_OBJ_PATH, exist_ok=True)
...
if not FLAGS.test_allset:
    img_path = FLAGS.img_path.split('_')
    VIEW_NUM = 3
    cat_id, modname, render1, render2, render3 = img_path[0], img_path[1], int(img_path[2]), int(img_path[3]), int(img_path[4])
    cats_limit[cat_id] += 1
    TEST_LISTINFO += [(cat_id, modname, [render1, render2, render3])]
else:
...
TEST_DATASET = data.Pt_sdf_img(FLAGS,
...
def create():
    ...
    input_pls = model.placeholder_inputs(BATCH_SIZE, NUM_POINTS, VIEW_NUM, (IMG_SIZE, IMG_SIZE),
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        for batch_idx in range(num_batches):
            ...
            if not FLAGS.threedcnn:
                for b in range(BATCH_SIZE):
                    ...
                    for j in range(SPLIT_SIZE):
                        ...
                        split_pointsf = split_pointsf * m + center[None, :] ### align with shapenet v1
            ...
            if FLAGS.binary:
                ...
                result = (prob - FLAGS.threshold) ####/ 10.
...
def create_obj(pred_sdf_val, sdf_params, dir, cat_id, obj_nm, view_id, i):
    if not isinstance(view_id, str):
        view_id = "%02d_%02d_%02d"%(view_id[0], view_id[1], view_id[2])
