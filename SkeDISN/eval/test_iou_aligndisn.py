...
parser.add_argument('--name', type=str, default='SkeGCNN', help='name of comparative method')
...
outfile_name = FLAGS.name + '_iou_aligndisn_res%d'%FLAGS.res
    ...
    with open(test_lst_f, "r") as f:
        ...
        for obj_id in test_objs:
            ...
            if len(pred_path_lst) == 0:
                ...
                continue
            pred_path = pred_path_lst[0]
            result_iou = iou_pymesh(cat_id, obj_id, src_path, pred_path)
            if result_iou>0:
                iou_sum += result_iou
                count += 1
                avg_iou = result_iou
...
def normalize_points_todisn(points, center, m):
    return (points - center[None, :]) / m

def iou_pymesh(cat_id, obj_id, mesh_src, mesh_pred):
    dim=FLAGS.res
    ...
    mesh2_alignv1 = pymesh.form_mesh((mesh2.vertices - centroid[None, :]) /m, mesh2.faces)
    grid2 = pymesh.VoxelGrid(2./dim)
    grid2.insert_mesh(mesh2_alignv1)
    grid2.create_grid()
