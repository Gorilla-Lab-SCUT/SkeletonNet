...
import model_add_skevox as model
import data_sdf_h5_queue_skevox_compress as data
import output_utils
...
### skeleton branch of Occuoancy Prediction
parser.add_argument('--ske_local_patch_share', action='store_true')
parser.add_argument('--free_onlylocal', action='store_true')
...
info = {'rendered_dir': raw_dirs["renderedh5_dir"],
        ...
        'ske_vox_dir': raw_dirs["ske_vox_dir"]}
if FLAGS.cam_est:
    info['rendered_dir']= raw_dirs["renderedh5_dir_est"]
if FLAGS.use_predvox:
    info['ske_vox_dir'] = FLAGS.predvox_dir
print(info)

TRAIN_DATASET = data.Pt_sdf_img(FLAGS, listinfo=TRAIN_LISTINFO, info=info, cats_limit=cats_limit)
...
def train():
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:0'):
            ...
            accuracy_sum = tf.summary.scalar('accuracy', end_points['losses']['accuracy'])
            realvalue_sum = tf.summary.scalar('realvalue', end_points['losses']['sdf_loss_realvalue'])
            ...
            merged = tf.summary.merge([accuracy_sum, realvalue_sum, loss_sum, lr_sum])
            train_writer = tf.summary.FileWriter(LOG_DIR, sess.graph)

            ##### all
            update_variables = [x for x in tf.get_collection_ref(tf.GraphKeys.GLOBAL_VARIABLES)]
            if FLAGS.free_onlylocal:
                if FLAGS.ske_local_patch_share:
                    update_variables = [x for x in tf.get_collection_ref(tf.GraphKeys.GLOBAL_VARIABLES)\
                    if x.op.name.startswith('local_patch_encode') or x.op.name.startswith('sdfprediction_voxfeat')]
            ...
            # load pretrained DISN
            if FLAGS.restore_modeldisn is not '':
                load_model(sess, FLAGS.restore_modeldisn, ['sdfprediction', 'sdfprediction_imgfeat', 'vgg_16'], strict=True)
                print('load sdfprediction, sdfprediction_imgfeat, vgg_16!!!')
            ...
            for epoch in range(MAX_EPOCH):
                ...
                save_path = saver.save(sess, os.path.join(LOG_DIR, "model_epoch.ckpt"))
                log_string("Model saved in file: %s" % save_path)
    ...
    for batch_idx in range(num_batches):
        ...
        if FLAGS.ske_local_patch_share:
            feed_dict = {ops['is_training_pl']: is_training,
                         ...
                         ####
                         ops['input_pls']['ske_vox64_patch4']:batch_data['vox64_patch4'],
                         ops['input_pls']['ske_vox128_patch4']:batch_data['vox128_patch4'],
                         ops['input_pls']['ske_vox256_patch4']:batch_data['vox256_patch4'],
                         ####
            ...
            feed_dict = {ops['is_training_pl']: is_training,
                         ...
                         ops['input_pls']['imgs']: batch_data['img'],
                         ops['input_pls']['trans_mat']: batch_data['trans_mat']}
        output_list = [ops['train_op'], ops['step'], ops['lr'], ops['loss'], ops['merged'],
        ...
        _, step, lr_val, loss_val, merged_scalars, ref_sdf_val, \
        sample_img_points_val, pred_sdf_val, ref_img_val = outputs[:-len(losses)]

        #pred_sdf_val /= SDF_WEIGHT
        train_writer.add_summary(merged_scalars, step)
