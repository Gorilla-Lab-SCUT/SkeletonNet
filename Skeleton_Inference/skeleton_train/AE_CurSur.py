...
parser.add_argument('--env', type=str, default ="AE_CurSur"   ,  help='visdom env')
...
parser.add_argument('--start_eval_epoch', type=float, default=150)
...
dataset = ShapeNet(train=True, class_choice =category_select, npoints_skeleton=opt.super_points, npoints_line = opt.super_points, npoints_square = opt.super_points, SVR=False, rotate=opt.rotate)
dataloader = torch.utils.data.DataLoader(dataset, batch_size=opt.batchSize, drop_last=True,
                                          shuffle=True, num_workers=int(opt.workers))
dataset_test = ShapeNet(train=False, class_choice =category_select, npoints_skeleton=opt.super_points, npoints_line = opt.super_points, npoints_square = opt.super_points, SVR=False, rotate=opt.rotate)
...
#Create network
network = AE_CurSur(num_points =opt.super_points, num_points_line = opt.num_points_line, num_points_square = opt.num_points_square,
...
for epoch in range(opt.nepoch):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        points_line_input = points_line.transpose(2,1)
        points_square_input = points_square.transpose(2,1)

        pointsReconstructed_cur, pointsReconstructed_sur = network.forward_inference(points_line_input, points_square_input, grid1, grid2)
