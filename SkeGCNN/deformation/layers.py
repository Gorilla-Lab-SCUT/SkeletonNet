...
class GraphProjection(Layer):
    def __init__(self, placeholders, **kwargs):
        ...
        self.trans_mat_right = placeholders['trans_mat']
        self.view_number = 1

    def _call(self, inputs):
        coord = inputs[:, 0:3]
        coord_camera = tf.matmul(coord, self.trans_mat_right[0:3, :]) + self.trans_mat_right[3:4, :]
        X = coord_camera[:, 0]
        Y = coord_camera[:, 1]
        Z = coord_camera[:, 2]


        h = 245.0 * tf.divide(Y, Z) + 111.5
        w = 245.0 * tf.divide(X, Z) + 111.5
